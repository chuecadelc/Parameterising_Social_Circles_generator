...
for rep in 1:iters 
  sc = Social_Circles(rows,cols,population,proportions[1][1],proportions[1][2],Random_sr)
  # replace run number for current iteration
  replace!(experiment_trials[!, :run_number], 0 => rep)  
  replace!(experiment_trials[!, :experiment], 0 => 3) 
  replace!(df_nodes[!, :run_number], 0 => rep)  
  replace!(df_nodes[!, :experiment], 0 => 3) 
